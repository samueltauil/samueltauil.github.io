...
- If `fetch_webpage` fails or returns an error for a repo URL, do not proceed with writing. Ask the user to paste the README and any relevant config files directly into the conversation before continuing.
...
1. **Cold open (1 to 2 short paragraphs).** Drop the reader into the moment. A customer said this. I opened VS Code and noticed that. Someone asked me a question I hear all the time. No preamble, no "In today's fast-paced world". Paragraph one has to end on unresolved tension: a question the reader now needs answered, not a summary of the answer. That gap is what earns the rest of the post. Vary the opening across posts: skim the last 3 to 5 entries in `_posts/` and avoid reusing the same hook shape (do not open with "Recently, I sat down with a customer" twice in a row, or "A customer asked me" three posts running). If the last 3 to 5 posts in `_posts/` cannot be read, skip the hook-variation check and note in the delivery summary that it was skipped due to unreadable files. Be a little creative with the storyline. A quick scene from a whiteboard, a moment on a plane, a Slack thread that would not die, the second time this week someone asked the same thing. As long as the scene is real and the tension is honest, the shape can shift.
